...
import { UploadedImage, Marketplace, ListingCondition } from "@/types";
...
import { useSmartAnalysis } from "@/services/ai";
import type { GeneratedListing } from "@/services/ai/types";
...
export function NewListing() {
  ...
  // Derived state from smart analysis
  const marketResearch = smartAnalysis?.marketResearch ?? null;
  const imageOrder = smartAnalysis?.imageOrder ?? null;
  ...
  const [aiGeneratedData, setAiGeneratedData] =
    useState<GeneratedListing | null>(null);
  ...
  // Apply smart analysis results — single effect handles everything
  useEffect(() => {
    ...
    const normalizeCondition = (value: string | undefined) => {
      ...
      const normalized = value
        .toLowerCase()
        .replace(/\bnew with tags\b/g, "new")
        .replace(/\bnew with box\b/g, "new")
        .replace(/\blike new\b/g, "like_new")
        .replace(/[\s-]+/g, "_")
        .trim();
      ...
      return validConditions.has(normalized) ? normalized : undefined;
    };

    // Image analysis → fill category/condition/brand
    if (ia) {
      if (!category && ia.suggestedCategory) setCategory(ia.suggestedCategory);
      if (!condition && ia.suggestedCondition) {
        const normalizedCondition = normalizeCondition(ia.suggestedCondition);
        if (normalizedCondition) setCondition(normalizedCondition);
      }
      if (!brand && ia.suggestedBrand) setBrand(ia.suggestedBrand);
    }

    // Image ordering → reorder the uploaded URLs
    if (io && io.length > 0 && uploadedImageUrls.length > 0) {
      const sorted = [...io].sort((a, b) => a.newPosition - b.newPosition);
      const reordered = sorted
        .map((o) => uploadedImageUrls[o.originalIndex])
        .filter(Boolean);
      if (reordered.length === uploadedImageUrls.length) {
        setOrderedImageUrls(reordered);
      }
    }

    // Generated listing → fill title/description/price/tags
    if (gl) {
      setAiGeneratedData(gl);
      if (autoGenerateAI) {
        setTitle(gl.title);
        setDescription(gl.description);
        if (gl.suggestedPrice && !price) {
          setPrice(gl.suggestedPrice.toString());
        }
      }
    }
  ...
  const handleUploadAndAnalyzeImages = useCallback(
    async (activeUser?: SupabaseUser | null) => {
      ...
      try {
        ...
        // Get the saved AI style preference
        const savedStyle =
          localStorage.getItem("ai-listing-style") || undefined;

        // Run the unified smart analysis pipeline
        // This handles: image analysis → ordering → market research → content generation
        await runAnalysis({
          imageUrls,
          title: title || undefined,
          price: price ? parseFloat(price) : undefined,
          writingStyle: savedStyle,
  ...
  // Re-run pipeline is no longer needed as a separate step.
  // The smart analysis pipeline handles everything in one shot.
  // Content generation is included in the pipeline result.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Use reordered URLs > uploaded URLs > preview URLs
      const imageUrls =
      ...
      const result = await addListing(currentUser.id, {
        ...
        category: category || undefined,
        condition: validCondition as ListingCondition | undefined,
        brand: brand || undefined,
        tags: (aiGeneratedData?.tags as string[] | undefined) || undefined,
        images: imageUrls,
        marketplaces: selectedMarketplaces,
        status: "published",
        // Attach smart analysis data for the Python worker
        ...(smartAnalysis
          ? {
              platformContent: smartAnalysis.platformContent as Record<
                string,
                unknown
              >,
              marketResearch: (smartAnalysis.marketResearch || undefined) as
      ...
      // Queue automated posting to Python worker via /api/listings/post
      // Worker uses browser-use with cloud browsers for autonomous posting
      if (selectedMarketplaces.length > 0) {
        try {
          const marketplaceIds =
            typeof selectedMarketplaces[0] === "string"
              ? selectedMarketplaces
              : selectedMarketplaces.map((m: unknown) =>
          ...
          if (queueResult.success) {
            alert(
              `🎉 Listing created and queued for posting!\n\n` +
                `Your listing will be automatically posted to ${selectedMarketplaces.length} marketplace(s) in the background.\n\n` +
                `Job ID: ${queueResult.jobId}\n\n` +
                `You can close this window - the worker will continue processing.`,
            );
          } else if (queueResult.missingMarketplaces) {
            // User needs to log into marketplaces via extension
            alert(
              `⚠️ Please log into these marketplaces first:\n\n` +
                `${queueResult.missingMarketplaces.join(", ")}\n\n` +
                `Use the browser extension to capture your login sessions, then try again.`,
            );
          } else {
            alert(
              `⚠️ Listing saved but automation queue failed: ${queueResult.error || "Unknown error"}\n\n` +
                `Make sure the Python worker service is running.`,
            );
          }
        } catch (automationError: unknown) {
          console.error("Automation queue error:", automationError);
          alert(
            `⚠️ Listing saved to database!\n\n` +
              `Automation queue error: ${automationError instanceof Error ? automationError.message : "Connection failed"}\n\n` +
              `Make sure Redis and the Python worker are running.`,
          );
        }
      } else {
        alert("🎉 Your listing has been saved successfully!");
      }
            ...
            <>
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {analyzing && !uploadingImages && (
                    <div className="flex items-center gap-2 text-sm text-primary-600 dark:text-primary-400">
                      <Loader2 className="h-4 w-4 animate-spin" />
                      <span>{currentStep || "AI analyzing..."}</span>
                    </div>
                  )}
                  {uploadedImageUrls.length > 0 &&
                    !uploadingImages &&
          ...
          <div className="flex items-center justify-between pt-4 border-t border-dark-200 dark:border-dark-800">
            ...
            <div className="flex items-center gap-3">
              {analyzing && (
                <div className="flex items-center gap-2 text-sm text-primary-600 dark:text-primary-400">
                  <Loader2 className="h-4 w-4 animate-spin" />
                  <span>{currentStep || "AI working..."}</span>
                </div>
              )}
              <button
                type="button"
            ...
            <div className="space-y-3">
              <div className="p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg space-y-2">
                <div className="flex items-center gap-2 text-green-700 dark:text-green-400 font-medium">
                  ...
                  {smartAnalysis?.pipeline && (
                    <span className="text-xs font-normal text-green-600 dark:text-green-500">
                      (
                      {(smartAnalysis.pipeline.totalDuration / 1000).toFixed(1)}
                      s)
                    </span>
                  )}
                </div>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  <span className="px-2 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded text-xs flex items-center gap-1">
                  ...
                  {imageOrder &&
                    imageOrder.some(
                      ...
                      <span className="px-2 py-1 bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 rounded text-xs flex items-center gap-1">
                        <ArrowUpDown className="h-3 w-3" />
                        Photos Reordered
                      </span>
                    )}
                  {marketResearch && marketResearch.numCompsFound > 0 && (
                    <span className="px-2 py-1 bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 rounded text-xs flex items-center gap-1">
                      <TrendingUp className="h-3 w-3" />
                      {marketResearch.numCompsFound} Comps Found
                    </span>
                  )}
                  <span className="px-2 py-1 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded text-xs flex items-center gap-1">
              ...
              {marketResearch && marketResearch.numCompsFound > 0 && (
                <div className="p-4 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800 rounded-lg space-y-3">
                  <div className="flex items-center gap-2 text-amber-800 dark:text-amber-300 font-medium text-sm">
                    ...
                    <span className="ml-auto text-xs font-normal px-2 py-0.5 bg-amber-200 dark:bg-amber-800 rounded-full">
                      {marketResearch.demandLevel} demand
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-3 text-center">
                    <div>
                      <p className="text-lg font-bold text-amber-900 dark:text-amber-200">
                        ${marketResearch.recommendedPrice.toFixed(0)}
                      </p>
                    ...
                    <div>
                      <p className="text-lg font-bold text-amber-900 dark:text-amber-200">
                        ${marketResearch.avgSoldPrice.toFixed(0)}
                      </p>
                      <p className="text-xs text-amber-700 dark:text-amber-400">
                        Avg Sold
                      </p>
                    ...
                    <div>
                      <p className="text-lg font-bold text-amber-900 dark:text-amber-200">
                        ${marketResearch.priceRangeLow.toFixed(0)}-$
                        {marketResearch.priceRangeHigh.toFixed(0)}
                      </p>
                      <p className="text-xs text-amber-700 dark:text-amber-400">
                        Price Range
                      </p>
                  ...
                  {marketResearch.trendingKeywords.length > 0 && (
                    <div className="flex flex-wrap gap-1.5">
                      {marketResearch.trendingKeywords
                        .slice(0, 8)
                        .map((kw, i) => (
                          <span
                            key={i}
                            className="px-1.5 py-0.5 bg-amber-200/60 dark:bg-amber-800/40 text-amber-800 dark:text-amber-300 rounded text-xs"
                          >
                            {kw}
                          </span>
                        ))}
