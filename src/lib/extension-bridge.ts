          ...
          (response: Record<string, unknown>) => {
            if (response?.success) {
              ...
              resolve({
                installed: true,
                connected: true,
                extensionId: (response.extensionId as string) || EXTENSION_ID,
              });
            }
          },
        );
      ...
      try {
        chrome.runtime.sendMessage(
          ...
          (response: Record<string, unknown>) => {
            clearTimeout(timeoutId);
            if (response?.success) {
              resolve((response.marketplaces as ExtensionMarketplaces) || {});
            } else {
              reject(
                new Error(
                  (response?.error as string) ||
                    "Failed to get marketplace statuses",
                ),
              );
            }
          },
      ...
      try {
        chrome.runtime.sendMessage(
          ...
          (response: Record<string, unknown>) => {
            clearTimeout(timeoutId);
            if (response?.success) {
              resolve((response.jobs as ListingJob[]) || []);
            } else {
              reject(
                new Error(
                  (response?.error as string) || "Failed to get pending jobs",
                ),
              );
            }
          },
