...
import { queueListingJob } from "@/services/queue/listings-queue";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { listingId, marketplaces, mode } = body;

    // Local mode: uses user's own Chrome (no sessions needed)
    const isLocalMode =
      mode === "local" || process.env.BROWSER_MODE === "local";
    ...
    // Check if user has sessions for requested marketplaces (skip in local mode)
    if (!isLocalMode) {
      ...
      if (sessionsError) {
        console.error(
          "[PostListing API] Failed to check sessions:",
          sessionsError,
        );
      }
      ...
      const sessionMarketplaces = new Set(
        validSessions.map((s) => s.marketplace),
      );
      ...
      console.log("[PostListing API] Local mode - skipping session check");
    }
    ...
    const queueResult = await queueListingJob({
      ...
      listing: {
        ...
        // Pass through smart analysis data if available
        platformContent: listing.platform_content || undefined,
        marketResearch: listing.market_research || undefined,
      },
      marketplaces,
    ...
    if (!queueResult.success) {
      console.error(
        "[PostListing API] Failed to queue job:",
        queueResult.error,
      );
    ...
    console.log(`[PostListing API] ✅ Job queued successfully: ${jobId}`);

    return NextResponse.json({
      success: true,
      jobId,
      message:
        "Listing job queued successfully. Worker will process it autonomously.",
      marketplaces,
