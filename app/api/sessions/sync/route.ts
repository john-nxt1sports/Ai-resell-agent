...
import { storeSession } from "@/lib/sessions/storage";
import { syncToCloudBrowser } from "@/lib/sessions/cloud-sync";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (authError || !user) {
      console.log("[SessionSync API] Unauthorized");
    ...
    const validMarketplaces = ["poshmark", "ebay", "mercari", "depop", "facebook"];
    ...
    if (!storeResult.success) {
      console.error("[SessionSync API] Failed to store session:", storeResult.error);
      return NextResponse.json(
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: sessions, error } = await supabase
      ...
      .select("marketplace, browser_profile_id, last_validated_at, expires_at, created_at")
      .eq("user_id", user.id)
